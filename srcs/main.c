...
////////
	...
	img_x.addr = mlx_get_data_addr(img_x.img, &img_x.bpp, &img_x.line_len, &img_x.endian);

	img_up.img = mlx_new_image(p.mlx, (img_w * 3), (img_h * 3));
	img_up.addr = mlx_get_data_addr(img_up.img, &img_up.bpp, &img_up.line_len, &img_up.endian);
	
	ft_upscale_img(img_w, img_h, 3, img_x.addr, img_up.addr);
//	my_mlx_pixel_put(&img_up, 1, 0, 0x00FF0000);
	mlx_put_image_to_window(p.mlx, p.win2, img_up.img, 10, 10);
